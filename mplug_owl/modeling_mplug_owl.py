...
class MplugOwlForConditionalGenerationModelOutput(ModelOutput):
    ...
    Args:
        loss (`torch.FloatTensor`, *optional*, returned when `labels` is provided, `torch.FloatTensor` of shape `(1,)`):
            Language modeling loss from the language mPLUG-Owl2.
        logits (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.vocab_size)`):
            Prediction scores of the language modeling head of the language mPLUG-Owl2.
        vision_outputs (`BaseModelOutputWithPooling`):
            Outputs of the vision encoder.

        language_model_outputs (`CausalLMOutputWithPast` or `Seq2SeqLMOutput`):
            Outputs of the language mPLUG-Owl2.
    """
...
def get_ltor_masks_and_position_ids_from_embeddings(data):
    """Build masks and position id for left to right mPLUG-Owl2."""
...
MPLUG_OWL_START_DOCSTRING = r"""
    This mPLUG-Owl2 inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its mPLUG-Owl2 (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)

    This mPLUG-Owl2 is also a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) subclass.
    Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
    and behavior.

    Parameters:
        config ([`MplugOwlConfig`]): Model configuration class with all the parameters of the mPLUG-Owl2.
            Initializing with a config file does not load the weights associated with the mPLUG-Owl2, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the mPLUG-Owl2 weights.
"""
...
MPLUG_OWL_INPUTS_DOCSTRING = r"""
    Args:
        ...
        input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
            Indices of input sequence tokens in the vocabulary of the language mPLUG-Owl2. Input tokens can optionally be
            provided to serve as text prompt, which the language mPLUG-Owl2 can continue.
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            Indices of decoder input sequence tokens in the vocabulary of the language mPLUG-Owl2. Only relevant in case an
            encoder-decoder language mPLUG-Owl2 (like T5) is used.
        ...
        decoder_attention_mask (`torch.BoolTensor` of shape `(batch_size, target_sequence_length)`, *optional*):
            ...
            Only relevant in case an encoder-decoder language mPLUG-Owl2 (like T5) is used.
...
class MplugOwlVisualAbstractorModel(MplugOwlPreTrainedModel):
    ...
    def _prune_heads(self, heads_to_prune):
        """
        Prunes heads of the mPLUG-Owl2. heads_to_prune: dict of {layer_num: list of heads to prune in this layer} See base
    ...
    def get_extended_attention_mask(
        ...
        Arguments:
            ...
            input_shape (`Tuple[int]`):
                The shape of the input to the mPLUG-Owl2.
            device: (`torch.device`):
                The device of the input to the mPLUG-Owl2.
        ...
        if attention_mask.dim() == 3:
            ...
            # - the mPLUG-Owl2 is an encoder, so make the mask broadcastable to [batch_size, num_heads, seq_length, seq_length]
    ...
    def forward(
        ...
        encoder_hidden_states  (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, `optional`):
            ...
            the mPLUG-Owl2 is configured as a decoder.
        encoder_attention_mask (`torch.FloatTensor` of shape `(batch_size, sequence_length)`, `optional`):
            Mask to avoid performing attention on the padding token indices of the encoder input. This mask is used in
            the cross-attention if the mPLUG-Owl2 is configured as a decoder. Mask values selected in `[0, 1]`:
            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        past_key_values (`tuple(tuple(torch.FloatTensor))` of length `config.n_layers` with each tuple having 4 tensors of:
            ...
            value states given to this mPLUG-Owl2) of shape `(batch_size, 1)` instead of all `decoder_input_ids` of shape
...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def _preprocess_accelerate(self):
        r"""
        Some pre-processing hacks to make the mPLUG-Owl2 `accelerate` compatible. Check
    ...
    def forward(
        ...
        >>> mPLUG-Owl2 = MplugOwlForConditionalGeneration.from_pretrained(
        ...     "MAGAer13/mplug-owl-llama-7b", torch_dtype=torch.float16
        ... )
        >>> mPLUG-Owl2.to(device)  # doctest: +IGNORE_RESULT
        ...
        >>> generated_ids = mPLUG-Owl2.generate(**inputs)
    ...
    def prepare_inputs_for_generation(
        ...
        # if mPLUG-Owl2 is used as a decoder in encoder-decoder mPLUG-Owl2, the decoder attention mask is created on the fly
        if attention_mask is None:
